...
export default function DashboardSimple({ serverUser }) {
...
export async function getServerSideProps(context) {
  try {
    console.log("Dashboard simple - getServerSideProps called");
    
    const session = await getServerSession(context.req, context.res, authOptions);
    console.log("Dashboard simple - session:", session);

    if (!session || !session.user) {
      console.log("Dashboard simple - No session, redirecting");
      return {
        redirect: {
          destination: "/",
          permanent: false,
    ...
    if (session.user.role !== "admin") {
      console.log("Dashboard simple - Not admin, redirecting");
      return {
        redirect: {
          destination: "/",
          permanent: false,
    ...
    console.log("Dashboard simple - Admin access granted");
