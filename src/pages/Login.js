import React from 'react'
import BreadCrumb from '../components/BreadCrumb'
import Meta from '../components/Meta';
import { Link } from 'react-router-dom'
import logo from '../../src/Image/logo.png'
import CustomInput from '../components/CustomInput';

const Login = () => {
  return (
    <>
      <Meta title={'Đăng nhập'} />
      <div className=''>
        <div className='container '>
          <div className='row'>
            <div className='col-12'>
              <div className='login-form d-flex'>
                <div className='col-6'>
                  <div className='login-img text-center text-white'>
                    <p className='login-p'> Welcome to</p>
                    <img src={logo} />
                    <p className='login-p'>
                      Thuận tự nhiên là tôn chỉ của chúng tôi trong quá trình chăn nuôi, trồng cấy các sản phẩm để cung cấp đến người tiêu dung tại hệ thống chuỗi thực phẩm sạch Eco BacGiang.
                    </p>
                  </div>
                </div>
                <div className='col-6'>
                  <div className='login-card'>
                    <h3 className='boder-content-h3 '> Đăng nhập</h3>
                    <form action='' style={{ marginTop: 80 }}  >
                      <div className='form-login'>
                        <CustomInput type='text' name = 'name' placeholder='Tên đăng nhập'  />
                        <CustomInput type='password' name = 'password' placeholder='Nhập mật khẩu'  />
                     
                      </div>
                      <div className=''>
                        <Link className='form' to='/quen-mat-khau'> Quên mật khẩu  </Link>
                        <Link className='signup' to='/dang-ky-tai-khoan'> Đăng ký ký tài khoản </Link>
                        <div className='d-flex justify-content-center gap-15 align-item-center'>
                          <button className='btn boder-0'> Đăng nhập </button>
                        </div>

                      </div>
                    </form>
                  </div>
                </div>
              </div>
            </div>
          </div>

        </div>
      </div>
    </>
  )
}

export default Login